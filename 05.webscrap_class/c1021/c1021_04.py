...
with open("C:/workspace/smclass/c1021/1.html","w",encoding="utf8") as f:
  f.write(res.text)
...
print(soup.title.text)          # title 태그 문자열을 출력 : text, get_text()
print("없는 태그 :",soup.titletitletitle)  # 없는 태그 입력시 None
# print("없는 태그 :",soup.titletitletitle.get_text())  # 없을시 에러발생
...
print(soup.find("div",{"id":"header"}))  # attrs 생략가능 / div태그 id = "header"
print(soup.find("h2",{"class":"rankingnews_tit"}).text)  # h2태그 class = "rankingnews_tit"의 텍스트
print(soup.find_all("div"))        # 모든 div태그 검색
print(len(soup.find_all("div")))   # 모든 div태그 개수 출력
print(type(soup.find_all("div")))  # ResultSet : 여러개의 묶음(객체의 리스트)


# soup.prettify() 정보 저장
# with open("C:/workspace/smclass/c1021/2.html","w",encoding="utf8") as f:
