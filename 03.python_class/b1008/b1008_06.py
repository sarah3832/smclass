...
# students.sort(key=lambda x:x[1])  # 이름 순 순차정렬 **(외우기)
# students.sort(key=lambda x:x[1], reverse=True)  # 이름 순 역순정렬
# x = sorted(students,key=lambda x:x[1])  # sorted 함수를 사용해서 정렬(딕셔너리는 sorted써야 정렬가능)
# print(students)
# print(x)
