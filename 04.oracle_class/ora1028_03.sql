...
no number,
name varchar2(20),
avg number,  -- 소수점 지정없이 선언
avg2 number(5,2)  -- 123.23(소수점 지정) / number(percision,scale)
);
...
select employee_id,emp_name,salary from employess;
select EMP_NAME from employees;

SELECT * EMP_NAME FROM EMPLOYEES;
select * emp_name from employees;

-- 밖 명령어는 대소문자 구분x / 속 내용은 구분o
select emp_name from employess where emp_name = 'pat Fay';

select * from employees;

-- 사원번호, 사원명, 전화번호, 입사일 컬럽만 출력하시오.
select EMPLOYEE_ID,emp_name,phone_number,hire_date from employees;

-- 사원번호, 사원명, 직급번호(job_id) SH_CLERK 사원만 출력하시오.
