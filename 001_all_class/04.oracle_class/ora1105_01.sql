-- join
-- inner joi - equi join, non-equi-join, self join, outer join

-- employees 사원번호, 사원명, 부서번호, 부서명-departments 출력하시오.
...
-- non-equi join : 두 테이블간 동일한 컬럼없이 데이터를 가져오는 방법 
...
-- avg의 컬럼을 가지고, stu_grade 활용해서 이 값을 result 입력하시오.
...
-- union : 같은테이블, 다른테이블 모두 사용이 가능하고 컬럼의 타입만 맞으면 모두출력, 개수도 맞아야함
-- 조건1 : 위쪽 쿼리문과 아래쪽 쿼리문 개수가 동일
-- 조건2 : 타입 무조건 일치
select employee_id no,emp_name name,manager_id from employees
union
...
-- employees에 salary 5000 and 8000 사원검색 -> 부서번호,부서이름
-- 두개를 union하시오.
select * from employees;
select * from departments;
...
-- employees department_id가 50인 사원검색 -> 부서번호,부서이름
-- employees에 없는 departments 부서검색 -> 부서번호,부서이름
-- 두개를 union하시오.
select a.department_id,department_name
from employees a,departments b 
where a.department_id = b.department_id
...
-- view : 복잡한 쿼리를 단순화, 필요한 정보만 접근하도록 제한
-- 상담원 : 사원들 전화번호를 가지고 사원들에게 마케팅 하려고 함.
-- 100명에게 사원테이블 오픈 제공해달라고 요청 : 마케팅
-- 직원가 100만원 90% 할인해서 10만원에 아이폰16 
select * from employees;
...
-- create : view 생성
create or replace view employees2_view
as
...
-- view 생성
-- with read only : select만 가능 / insert,update,delete 불가능
...
-- insert, update,delete 가능 / not null 제약조건이 되어있으면  insert 불가할수도있음
-- 복합 view  : 2개이상 테이블조인, 함수사용, group by 같은 경우는 insert, update,delete 불가
-- update : 100번 이름을 홍길동으로 변경
...
select no,name,kor,eng,math,total,round(avg,2),rank,to_char(sdate,'yyyy-mm-dd') from students order by total;